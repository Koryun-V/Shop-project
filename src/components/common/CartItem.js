...
const CartItem = ({card, selectedProducts, setSelectedProducts, setCheckedAll}) => {
  ...
  const onChange = async ({quantity,quantityProduct, cardId, value}) => {
    console.log(quantity,quantityProduct,"fgiobj")
    const numericValue = value - localQuantity
    if (quantity > quantityProduct){
      toast.success("This quantity is not available. Please delete or reduce the quantity.")
    }
    if (/^\d*$/.test(value)) {
        ...
        <div className="p">
          <div className="card-prod">
            ...
            {card.product.price * localQuantity}$
              ...
              {!_.isEmpty(card.product.discount) &&
                  <>
                    <div>
                  < span
                      className="card-price discountPercentage">{parseFloat(card.product?.discount?.discountPercentage)}%</span>
            ...
            <div className="quantity">
                ...
                <input
                    ...
                    onChange={({ target: { value } }) =>
                        onChange({
                          ...
                          quantityProduct: card.product.quantity,
                          value: Number(value),
                        })
                    }
              ...
              <Button
                  className="active-button decrement"
                  disabled={loading && updating.includes(card.id) || card.quantity >= card.product.quantity}
                  onClick={() => updateQuantity({cardId: card.id, value: 1})}
