...
const Products = () => {
    ...
    useEffect(() => {
        if (categoryId) {
            const idsArray = categoryIds
                .split(",")
                .map((item) => Number(item.trim()))
                .filter(Boolean);

            if (!idsArray.includes(categoryId)) {
                const newIds = [...idsArray, categoryId];
                const newCategoryIds = newIds.join(",");
                ...
                if (searchValue.trim()) updatedQuery.s = searchValue.trim();
                if (storeId) updatedQuery.storeId = storeId;
                if (userId) updatedQuery.userId = userId;

                setQuery(updatedQuery);
                dispatch(getAllProducts(updatedQuery));
            }
        }
    }, [categoryId]);
