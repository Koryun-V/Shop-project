...
const fields = [
    ...
    {id: 3, name: "day", label: "Day", validation: /^(0[1-9]|[12][0-9]|3[01])$/, maxLength: "2"},
    {id: 4, name: "month", label: "Month", validation: /^(0[1-9]|1[0-2])$/, maxLength: "2"},
    {id: 5, name: "year", label: "Year", validation: /^(19\d{2}|20(0[0-9]|1[0-4]))$/, maxLength: "4"},
    {
        id: 6,
        name: "email",
        label: "E-mail",
    ...
    {id: 8, name: "repeatPassword", label: "RP/password", validation: "", info: "Password don't match."},
];
...
const Register = () => {
    ...
    const [userInfo, setUserInfo] = useState({value: "", title: ""})
    ...
    useEffect(() => {
        if (statusKey === "ok") {
            const time = setTimeout(() => setIsCheck(true), 1000);
    ...
    useEffect(() => {
        if (isCheck) {
            const time = setTimeout(() => setIsAnimation(true), 100);
            return () => clearTimeout(time)
        }
    }, [isCheck]);
    ...
    const test = () => {
        ...
        fields.forEach(({validation, name, info}) => {
            if (title === name) {
                let isValid = true;
                ...
                if (!isValid || !value.length) {
                    if (!newInputName.includes(name)) newInputName.push(name);
                    ...
                    if (passwordValid && passwordsMatch) {
                        ...
                        if (!passwordValid && !newInputName.includes("password")) {
                            newInputName.push("password");
                        }
                        if (!passwordsMatch && !newInputName.includes("repeatPassword")) {
                    ...
                    if (user.day.length === 2 && user.month.length === 2 && user.year.length === 4) {
                        if (isDate === "ok") {
                            newInputName = newInputName.filter(item =>
                                item !== "day" && item !== "month" && item !== "year"
                            );
                        } else {
                            if (!newInputName.includes("day")) newInputName.push("day");
                            if (!newInputName.includes("month")) newInputName.push("month");
                            if (!newInputName.includes("year")) newInputName.push("year");
                        }
                    }
                } else {
                    ...
                    <div className="container-register">
                        <div className="container-form" style={{width: 320}}>
                            ...
                            {status !== "ok" ? <form onSubmit={register}>
                                        ...
                                        <div className="field-block" key={field.id}>
                                            ...
                                            <div className="validation-info">
                                                {statusRegister === "active" && field.name === "email" ?
                                                    ...
                                                    <div>
                                                        <div className="test2"></div>
                                                        <span>{!user[field.name].length ? "Field Required" : field.info}</span>
                                                    </div>
                                                }
                                            </div>
                                        </div>
                                    ...
                                    <div className="form-button-block" style={{marginTop: 20}}>
                                        <Button
                                            status={statusDelete === "pending" ? statusDelete : status}
                                            text="CONTINUE"
                                            type={isRegister ? "submit" : "button"}
                                            className={isRegister && status !== "pending"
                                            || isRegister && statusDelete === "pending" ? "active-button"
                                                : isRegister && (status === "pending" || statusDelete === "pending") ? "pending-button"
                                                    : "disabled"}
                                        >Text</Button>
                                    </div>
                                </form>
                                : !isCheck ? (
                                    <div className="container-form" style={{marginTop: 50}}>
                                        ...
                                        <div className="email-text">
                                            ...
                                            <span>Enter the confirmation code we sent to {email}</span>
                                        </div>
                                        <div className="pin-block"><PinInput/></div>
                                        <div className="timer"><Timer email={email}/></div>
                                    </div>
                                ) : (
                        ...
                        <div className="status-block">
                            <div className="status-register">
                                <div className="line-status" style={{
                                    height: status === "ok" && isCheck && statusKey === "ok" ? "100%" :
                                        status === "ok" ? "50%" : 0
                                }}></div>
                            </div>
                            <div className="circle"></div>
                            <div className="circle" style={{background: status === "ok" ? "#00d143" : "#979797"}}></div>
                            <div className="circle"
                                 style={{background: statusKey === "ok" ? "#00d143" : "#979797"}}></div>
                        </div>
                    </div>
                    <div className="container-register-img">
                        <img src={bg} className="register-img" alt="register background"/>
                    </div>
                </div>
            </div>
        </div>
